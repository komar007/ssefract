...
SSE-accelerated Burning Ship Fractal renderer with SDL browser. The core iteration loop is written
in hand-optimized x86 assembly (SSE2 + x87), with a C implementation for comparison.

*University project.*

## Quick start

1.  Run the interactive browser (requires a display)

``` sh
nix run github:komar007/ssefract
```

2.  Batch render a BMP from the command line

``` sh
nix run github:komar007/ssefract#render -- output.bmp 1920 1080 -2.0 -1.2 0.8 1.2 50 200 colorpalette1.pal 000000 8
```

3.  Benchmark the ASM vs C implementations
...
- `fractal.asm` — hand-written SSE2 + x87 implementation (32-bit x86)
